...
var xpay_ui = {
  AD_URL: 'https://one.mofanbaby.com/v1/ad',
  ...
  resume: function () {
    xpay.createPayment(stash.charge, bind.callbackCharge);
  },
  ...
  createPayment: function(data, callback) {
    ...
    try{
      localStorage.setItem('xpay_app_id', data.app);
      localStorage.setItem('xpay_ch_id', data.id);
      localStorage.setItem('xpay_amount', data.amount);
      localStorage.setItem('xpay_subject', data.subject);
      localStorage.setItem('xpay_channel', data.channel);
    } catch (e) { /* empty */}

    xpay.createPayment(data, callback);
  },
  ...
  success: function (callback, continueCallback) {
    ...
    comUtil.documentReady(function() {
      try {
        stash.app_id = localStorage.getItem('xpay_app_id');
        stash.ch_id = localStorage.getItem('xpay_ch_id');
        stash.amount = localStorage.getItem('xpay_amount');
        stash.subject = localStorage.getItem('xpay_subject');
        stash.channel = localStorage.getItem('xpay_channel');
      } catch (e) { /* empty */ }

      /*global xpay_app_id,xpay_ch_id,xpay_amount,xpay_subject,xpay_channel*/
      if(!stash.app_id && ('undefined' != typeof xpay_app_id)){
        stash.app_id = xpay_app_id;
      }
      if(!stash.ch_id && ('undefined' != typeof xpay_ch_id)) {
        stash.ch_id = xpay_ch_id;
      }
      if(!stash.amount && ('undefined' != typeof xpay_amount)) {
        stash.amount = xpay_amount;
      }
      if(!stash.subject && ('undefined' != typeof xpay_subject)) {
        stash.subject = xpay_subject;
      }
      if(!stash.channel && ('undefined' != typeof xpay_channel)) {
        stash.channel = xpay_channel;
      }
      var device = 'H5';
      if(comUtil.inAlipay()) {
        device = 'alipay';
...
if (typeof window != 'undefined') {
  window.xpay_ui = xpay_ui;
  comUtil.documentReady(function () {
    setTimeout(function () {
      var e = document.createEvent('Event');
      e.initEvent('xpay_one_ready', true, true);
      document.dispatchEvent(e);
    }, 0);
  });
}

module.exports = xpay_ui;
